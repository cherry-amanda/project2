        ...
        <div class="col-md-4 mb-4">
            <div class="card h-100 shadow-sm border-0">
                ...
                <div class="card-body">
                    ...
                    <h6 class="fw-bold text-success">Rp {{ number_format($package->harga_total, 0, ',', '.') }}</h6>

                    @if($package->type === 'paket')
                        <table class="table table-sm mt-3 mb-2 border">
                            ...
                            <tbody>
                                @foreach($package->packageRabs as $rab)
                                    <tr>
                                        <td class="small">{{ $rab->vendorService->nama_item ?? '-' }}</td>
                                        <td class="small text-end">Rp {{ number_format($rab->harga_item, 0, ',', '.') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    <div class="d-flex justify-content-between mt-3">
                        <a href="{{ route('admin.package.show', $package->id) }}" class="btn btn-outline-primary btn-sm w-100 me-1" title="Detail">
                            <i class="bi bi-eye"></i> Detail
                        </a>
                        <a href="{{ route('admin.package.edit', $package->id) }}" class="btn btn-outline-warning btn-sm w-100 me-1" title="Edit">
                            <i class="bi bi-pencil"></i> Edit
                        </a>
                        <form method="POST" action="{{ route('admin.package.destroy', $package->id) }}" class="w-100 form-delete">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-outline-danger btn-sm w-100" title="Hapus">
